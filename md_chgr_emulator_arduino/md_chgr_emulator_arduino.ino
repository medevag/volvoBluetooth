 ...
 * 
 * The HU enables the CD-CHGR in its source-menue after a successful initialization procedure is accomplis
 */
...
void loop() {
  ...
  if (ByteIsRead) {
    ...
    if(melbus_LastReadByte[0x02] == 0x07 && melbus_LastReadByte[0x01] == 0x1A && melbus_LastReadByte[0] == 0xEE)
    {
    ...
    if(melbus_LastReadByte[0] == 0xD8 && InitialSequence == true){
...
void melbus_Init_CDCHRG() {
 ...
 // Wait untill Busy-line goes high (not busy) before we pull BUSY low to request init
  ...
  Serial.println("Leaving initation");
